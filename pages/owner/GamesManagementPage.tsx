...
const GamesManagementPage: React.FC = () => {
    ...
    if (loading) return <p className="text-gray-300 text-center py-10">Loading screens...</p>;

    return (
        <div className="mt-8">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-4xl font-bold text-gray-200">Screens Menu 🎬</h1>
            </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
                    <div className="bg-gray-800 border border-gray-700 p-8 rounded-lg shadow-xl w-full max-w-lg animate-fade-in-down max-h-[90vh] overflow-y-auto scrollbar-thin">
                        <h2 className="text-2xl font-bold mb-6 text-white">{editingItem ? 'Edit Screen' : 'Add New Screen'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-4 text-gray-300">
                            ...
                            <InputField 
                                label="Available Slots / Rooms" 
                                name="slotsString" 
                                value={formData.slotsString} 
                                onChange={handleInputChange} 
                                placeholder="e.g., SCREEN 1, SCREEN 2" 
                            />
                            <p className="text-xs text-gray-500 -mt-3 mb-2">Enter available slots separated by commas.</p>
                            
                             <div>
                                <label className="block text-gray-300 font-semibold mb-2">Screen Image</label>
                            ...
                            <CheckboxField label="Screen is available for booking" name="isAvailable" checked={formData.isAvailable} onChange={handleInputChange} />
